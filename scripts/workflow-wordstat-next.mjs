...
/** Windows CI/десктоп — npm.cmd; Linux/macOS cron — npm */
...
run(npmCli, ["run", "wp:sync-content-index"]);
...
if (!reusePending || existingLast?.mode !== "topic" || !String(existingLast?.phrase ?? "").trim()) {
  run(npmCli, ["run", "wp:wordstat-queue-next"]);
}
...
run(npmCli, ["run", "workflow:cloud", "--", phrase]);
