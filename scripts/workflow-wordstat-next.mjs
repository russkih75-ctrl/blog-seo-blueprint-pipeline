...
/** Windows CI uses npm.cmd; Linux/cron uses npm. */
...
run(npmExec, ["run", "wp:sync-content-index"]);
...
if (!reusePending || existingLast?.mode !== "topic" || !String(existingLast?.phrase ?? "").trim()) {
  run(npmExec, ["run", "wp:wordstat-queue-next"]);
}
...
run(npmExec, ["run", "workflow:cloud", "--", phrase]);
