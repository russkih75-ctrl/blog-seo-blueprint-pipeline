...
**Состояние очереди между Cloud-запусками:** после verified publication агент должен **закоммитить** обновление **`data/wordstat-published-keywords.json`** (без секретов). Этот файл синхронизируется из **`npm run content:finalize-publish`** и исключает повторный выбор уже опубликованного ключа при пустом `artifacts/`.
