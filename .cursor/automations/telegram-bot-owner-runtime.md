...
Функции бота для владельца (после allowlist): режимы /ask · /plan · /agent; один процесс бота может обслуживать два сайта — профиль на чат: /site · /site_wordprais · /site_bytmaster34 (изолированные очереди Wordstat и состояние пайплайна); публикация статьи из очереди только после явного /publish_article_confirm; /stop_automation выключает локальные расписания и напоминает выключить Cursor Cloud Automations вручную в UI (прямая ссылка приходит в Telegram).
```
