...
1) Установи зависимости и собери проект:
   ...
   (TELEGRAM_BOT_TOKEN должен быть set, чтобы бот запускался; CURSOR_API_KEY — set для ответов агента на обычный текст; статусы только set/empty/missing, без значений)
